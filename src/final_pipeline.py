...
class DirectorStyleTransfer:
    ...
    def process_video(self, input_video_path, output_video_path, 
        ...
        if evaluate and evaluator is not None and len(output_frames_for_eval) > 0:
            ...
            # Load target frames
            target_frames = []
            frames_dir = f"Frames/{self.director_name}" if not self.reference_path else None
            ...
            if frames_dir and os.path.exists(frames_dir):
                ...
                sample_indices = np.linspace(0, len(all_target_frames) - 1, 
                                            ...
                                            dtype=int)
                for idx in sample_indices:
                    target_path = os.path.join(frames_dir, all_target_frames[idx])
            ...
            if len(target_frames) > 0:
                ...
                if len(output_frames_for_eval) > 1:
                    temporal_metrics = evaluator.evaluate_temporal_stability(
                        output_frames_for_eval, 
                        verbose=True
                    )
    ...
    def process_image(self, input_image_path, output_image_path, 
        ...
        if evaluate:
            ...
            if target is not None:
                ...
                evaluator.save_metrics(f"{eval_output_dir}/{output_base}_metrics.json")
                evaluator.plot_metrics(f"{eval_output_dir}/{output_base}_metrics.png")
    ...
    def _load_sample_target_image(self):
        ...
        if self.reference_path:
            ...
            if os.path.exists(frames_dir):
                ...
                if frames:
                    sample_path = os.path.join(frames_dir, frames[len(frames)//2])  # Middle frame
...
def main():
    ...
    # Initialize pipeline with reference or director
    if args.reference:
